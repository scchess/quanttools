...
PlotDts <- R6::R6Class(
  ...
  public = list(
    ...
    limits = function( tlim = NULL, ylim = NULL ) {
    ...
    lines = function( names = 'auto', labels = names, ... ) {

      if( all( names == 'auto' ) )  {

        names = private$data_summary[ class %in% c( 'numeric', 'double', 'integer' ), name ]
        labels = names
    ...
    candles = function( names = c( 'open', 'high', 'low', 'close' ), label = 'candles', ... ) {

      private$add_task( type = 'candles', c( as.list( environment() ), list( args = list( ... ) ) ) )
      ( self )

    },
    events = function( name, label = name, ... ) {

      private$add_task( type = 'events', c( as.list( environment() ), list( args = list( ... ) ) ) )
      ( self )

    },
    histogram = function( names, labels = names, y0 = 0, ... ) {
    ...
    segments = function( names, label = 'segments', ... ) {
  ...
  private = list(
    ...
    calc_basis = function( t, gap = 5, fixed = T, range = NULL, units = 'mins' ) {
    ...
    set_limits = function( tlim = NULL, ylim = NULL ) {
    ...
    calc_intraday_marks = function( b, step ) {
    ...
    plot_value_axis_and_grid = function() {
    ...
    plot_time_axis_and_grid = function() {
    ...
    plot_candles = function( x, y, time_frame, col = 'auto', type = c( 'barchart', 'candlestick' ), gap = 0.2, border = NA ) {
    ...
    plot_events = function( x, labels = NA, lty = 1, col = 'auto', lwd = 1 ) {
    ...
    plot_lines = function( x, y, type = 'l', lty = 1, pch = NA, col = 'auto', bg = NA, lwd = 1, lend = 'round' ) {
    ...
    plot_segments = function( x0, y0, x1 = x0, y1 = y0, col = 'auto', lty = 1, lwd = 1 ) {
    ...
    plot_histogram = function( x, y, gap = 0.2, col = 'auto', border = NA, time_frame = NULL, split = F, stacked = F, gap2 = 0.05, y0 = 0 ) {
